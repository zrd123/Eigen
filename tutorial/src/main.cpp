...
static void matrix_mul_matrix(T* p1, int iRow1, int iCol1, T* p2, int iRow2, int iCol2, T* p3)
{
    ...
    Eigen::Map< Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> > map3(p3, iRow1, iCol2);
...
int main(int argc, char* argv[])
{
    ...
    //5. 重置矩阵大小: resize()
    // 不能修改静态矩阵的大小， 同时resize会析构原来的数据
    // 使用“=”操作符操作动态矩阵时，左边矩阵会被调整为右边矩阵的大小
    // 对于小矩阵（一般大小小于16）的使用固定大小的静态矩阵，它可以带来比较高的效率，对于大矩阵（一般大小大于32）建议使用动态矩阵
    Eigen::MatrixXd matrixXd1(3, 3);
    m = matrixXd1;
    std::cout << m.rows() << "  " << m.cols() << "  " << m.size() << std::endl << std::endl;
    ...
    matrix_mul_matrix(array1, 2, 3, array2, 3, 2, array3);
    for (int i = 0; i < 4; i++){
